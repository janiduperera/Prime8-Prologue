...
namespace Narrate
{
    public class SubtitleManager : MonoBehaviour
    {
        ...
        public float timePadding = 1.0f;// When finished scrolling, will display for this much longer
        ...
        public void DisplaySubtitle(string sub, float displayFor = -1, bool hideOnFinish = true, string sp = null)
        {

            _autherImageLocation = sp;
            //Janidu Deleted
            //if subtitles are not on, don't do anything
            //if (!NarrationManager.TextMode && PlayerPrefs.GetInt (SubtitleManager.PrefsKey) != 1) {

            //  Debug.LogWarning (!NarrationManager.TextMode);
            //  Debug.LogWarning (PlayerPrefs.GetInt (SubtitleManager.PrefsKey));
            //  return;
            //}

            if (OnSubtitileDisplayBeginEvent != null)
            {
                OnSubtitileDisplayBeginEvent.Invoke();
            }
        ...
        public void DisplayCleanup()
        {
            ...
            //if (GameController.Instance.GameState == GameStates.QuestionSet_1_Start) { // Start The First Q & A
            //  GameController.Instance.GameState = GameStates.QuestionSet_1_Started;
            //  GameController.Instance.CallChangeGameStates ();
            //}

            if (OnSubtitileDisplayEndEvent != null)
            {
                OnSubtitileDisplayEndEvent.Invoke();
            }
        }

        public delegate void OnSubtitileDisplayBegin();
        public event OnSubtitileDisplayBegin OnSubtitileDisplayBeginEvent;

        public delegate void OnSubtitileDisplayEnd();
        public event OnSubtitileDisplayEnd OnSubtitileDisplayEndEvent;
        //      IEnumerator StartQuestions(int _min, int _max)
        //      {
        //          yield return new WaitForSeconds (0.3f);
        //          GameController.Instance.InitiateQuestion (_min, _max);
        //          GameController.Instance.ActivateOrDeactivateFPSOfActivePlayer (false);
        //          GameController.Instance.GetCursorForUI ();
        //      }
    }
}
